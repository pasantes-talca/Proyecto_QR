...
# =======================
def get_app_dir():
    """Directorio del .exe (o del .py en desarrollo)."""
...
def get_config_dir():
    """
    Carpeta donde se guarda config.json.

    - En Windows  → C:\\Users\\<usuario>\\AppData\\Roaming\\TalcaQR\\
    - En macOS    → ~/Library/Application Support/TalcaQR/
    - En Linux    → ~/.config/TalcaQR/

    Usar AppData (y equivalentes) garantiza permisos de escritura en
    CUALQUIER PC, sin importar dónde esté instalado el .exe.
    Si por algún motivo falla, cae al directorio del ejecutable como respaldo.
    """
    try:
        ...
        config_dir = os.path.join(base, "TalcaQR")
        os.makedirs(config_dir, exist_ok=True)
        return config_dir
    except Exception:
        # Respaldo: misma carpeta que el ejecutable
        return get_app_dir()
...
def save_config(data: dict):
    try:
        ...
        # Error visible: si no se puede guardar, el usuario tiene que saberlo
        try:
            messagebox.showerror(
                ...
                f"  {CONFIG_FILE}\n\n"
                f"Error: {e}\n\n"
                f"La caché NO se guardó. Al cerrar la app perderás el estado actual."
            )
        except Exception:
...
def main():
    ...
    def al_hacer_click_resetear():
        ...
        confirmar = messagebox.askyesno(
            "⚠️  ¿Estás seguro?",
            f"Vas a resetear la caché del producto:\n\n"
            f"  {desc}\n\n"
        ...
        if confirmar:
            ...
            messagebox.showinfo(
                "Caché reseteada",
                f"✅ La caché de '{desc}' fue reseteada.\n"
    ...
    # ── Ruta del config (informativo) ────────────────────────────────────────
    tb.Label(
        root,
        text=f"📁 Caché guardada en: {CONFIG_FILE}",
        font=("Segoe UI", 8),
        bootstyle="secondary"
